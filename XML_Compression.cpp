...
/*
    ...
    {
    	//cout<<"traverse_huff"<<"\n";
    	HuffmanDict[node->character] = (str != "") ? str : "1";
    	cout<<node->character<<" => "<<HuffmanDict[node->character]<<"\n";
    }
...
/*
//Get number of bytes in a string (EXTRA)
long long Num_of_Bytes(string s)
{
	long long temp_num = stoll(s, 0, 2);
	long long number_of_bytes = 0;
	while (temp_num != 0)
	{
		temp_num >>= 8;
		number_of_bytes++;
	...
	//may replace it with str.length()/8 later
}
*/
...
void PrintCompressedTree(const vector<string> &XML_fixed, int size)
{
	...
	/*
	vector<unsigned int> fileByteArr;
	string_to_intArr(file_string , fileByteArr);
	long long fileByteArr_Size = fileByteArr.size();
	*/
	...
	{
		//unsigned int key = fileByteArr[i];

...
void XML_Parser(vector<string> &OpenTags, int &lines, vector<string> &XML_Original, vector<string> &XML_Fix)
{
	...
	do
	{
		...
		else
		{
			...
			//t = OpenTags[OpenTagIndex];
			/*if (t[t.length()-1]=='/')						// Self Closing Tags
			{
				lines++;
				continue;
			}*/
...
/*void count(vector<string> &tags, vector<int> &order, vector<bool> &puttagname)
{
	for (int i = tags.size() - 1; i >= 0; i--)
	{
		if (i != tags.size - 1 && tags[i] == tags[i + 1])
		{
			order[i] = order[i + 1] + 1;
		}
		else
